...
//        atexit(dumpregs);
	...
	Genode::memset(&arm, 0, sizeof(arm));
...
arm_unpredictable(uint32_t opcode)
{
	...
	if (unpredictable_count != 0) {
		unpredictable_count--;
		printf("ARM: Unpredictable opcode %08x at %08x\n", opcode, PC);
	}
}
