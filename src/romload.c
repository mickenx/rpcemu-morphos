...
void loadroms(void)
{
	...
	if (dir != NULL) {
		while ((d = readdir(dir)) != NULL && number_of_files < MAXROMS) {
			...
			strcpy(filepath,d->d_name);
			if (strcmp(d->d_name,"riscos.rom")) {
	...
	printf("number files: %d\n",number_of_files);
        ...
        for (c = 0; c < number_of_files; c++) {
                ...
                long len;
                char filepath[512];
		//if (!strcmp(romfilenames[c],"riscos.rom")) {
                //snprintf(filepath, sizeof(filepath), "%s%s", romdirectory, romfilenames[c]);
		//strcpy(filepath,romfilenames[c]);
		strcpy(filepath,"roms/riscos.rom");
		printf("filepath: %s\n",filepath);
                f = fopen(filepath, "rb");
                if (f == NULL) {
		...
		printf("fread1\n");
                if (fread(&romb[pos], (size_t) len, 1, f) != 1) {
                        printf("Error reading from ROM file '%s'\n",
                              romfilenames[c]);
                }
		...
		printf("fclose\n");
		printf("romload: Loaded '%s' %ld bytes\n", romfilenames[c], len);
	...
	printf("romload: Total ROM size %d MB\n", pos / 1048576);
