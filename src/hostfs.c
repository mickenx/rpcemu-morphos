                 ...
                 risc_os_object_info *object_info)
{
  ...
  //char tpath [1024];
  //strcpy(tpath,"/hostfs/");
  //strcat(tpath,host_dir_path);
  //strcpy(host_dir_path,tpath);
...
hostfs_init(void)
{
  ...
  //sprintf(HOSTFS_ROOT, sizeof(HOSTFS_ROOT), "%shostfs", "/");//rpcemu_get_datadir());
  strcpy(HOSTFS_ROOT,"");
  for (c = 0; c < 511; c++) {
    if (HOSTFS_ROOT[c] == '\\') {
      HOSTFS_ROOT[c] = '/';
